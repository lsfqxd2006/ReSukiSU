...
static void security_dump_masked_av(struct context *scontext, struct context *tcontext, u16 tclass, u32 permissions,
                                    const char *reason) static int constraint_expr_eval(struct context *scontext,
...
static inline u32 current_sid(void) static int string_to_context_struct(struct policydb *pol, struct sidtab *sidtabp,
...
static int ksu_selinux_hide_enable()
{
    ...
    backup_policydb = &fake_state.ss->policydb;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 0, 0) || defined(KSU_COMPAT_SIDTAB_AS_REFERENCE)
    backup_sidtab = fake_state.ss->sidtab;
#else
    backup_sidtab = &fake_state.ss->sidtab;
#endif
